    ...
    m("nav.navbar.navbar-default[role='navigation']", [
      m(".navbar-header", [
        m(
          ...
          [m("span.glyphicon.glyphicon-trash"), "Clear cache"]
        ),
        m(
          "button.goto.btn.btn-primary",
          {
            onclick: () => {
              if (state.gitlabPath) {
          ...
          [m("i.fa.fa-gitlab[aria-hidden='true']"), "Go to GitLab"]
        ),
        m(
          "button.goto.btn.btn-success",
          {
            onclick: () => {
              state.markAllAsRead();
            }
          },
          [m("i.fa.fa-check[aria-hidden='true']"), "Mark all as read"]
        ),
        m(
          "button.ciTrigger.btn.btn-info",
          {
            onclick: () => {
              window.open("trigger.html");
            }
          },
          [m("i.fa.fa-send[aria-hidden='true']"), "ci trigger"]
        )
      ])
    ]),
    m(
      "ul[id='notifyHistories']",
      state.histories.map(project_event => {
        ...
        let li_class = null;
        if (
          current_time.getTime() - state.new_milli_seconds <
          new Date(project_event.notified_at).getTime()
        ) {
          li_class = "new";
        }
        ...
        return m("li", { class: li_class }, [
          ...
          m(
            "span.label",
            { class: action_labels[project_event.action_name] },
            project_event.action_name
          ),
          m("span", " "),
          m(
            "span.timeago",
          ...
          m(
            "a.eventLink",
            {
              href: app.sanitizeUrl(project_event.target_url),
              target: "_blank"
            },
            message
          )
